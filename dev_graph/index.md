...
Last updated: 2026-06-25 (**ISSUE-07 ops console LIVE read-model:** the operator console now renders the **live** ledger/portfolio/reconcile state (ADR-014's separate `*.live` files), not only the SIM panels. Additive Tier-1 read-only over `ops/core.py` (`live_ledger_view`/`live_portfolio_view`/`live_operational_view` via path-parameterized reuse; new `reconcile_view` with a DERIVED `execution_refused`/`adoptable` state; `pending_orders_view`; `live_state_view` with the plug/kill-switch/refuse banner; `SIM_BADGE`/`LIVE_BADGE` so the accumulate-only SIM portfolio — a *model number, not performance* — is never read as a track record) + `ops/app.py` (a top live-state strip, a **Live** tab, SIM badges on the canonical panels). One governed **Tier-3** control added in `ops/gated.py`: **adopt-broker-position** (confirm + audit + server-side precondition) adopts an observed broker position into the live portfolio — **append-only reconcile-adopt, never automatic** (ADR-014 §6.2) via the existing immutable `PortfolioState` (`append_reconcile` + `dataclasses.replace` + `persist_portfolio`) — clearing the terminal-refuse. **No `src/`/contract/`*_version` change, no determinism-path impact:** BENCH-004/006 regenerated **byte-identical** (`git hash-object` unchanged). A 15-agent adversarial review found + **fixed 5 confirmed findings** — the load-bearing one a MAJOR: adopt hard-coded `realized_pnl=0.0` and dropped the prior same-instrument position, silently wiping accumulated LIVE realized P&L after a prior round-trip → now carries the realized accumulator forward (mirrors `_apply_live_buy`); plus 3 minor honesty/badging fixes (SIM-scope the always-on pipeline figures; banner labelled advisory + honest in the multi-instrument mixed-discrepancy case). +1 node (TEST-041 [[test_live_monitoring]], 24 tests incl. 2 review regressions); full suite **1168 green**, `mypy --strict`/`ruff` clean on touched files (the 2 remaining mypy errors are pre-existing in the untouched `feature_builder.py`). **Prior —** **ADR-014 pre-live-run readiness pass:** *armed* the adapter by closing the two remaining live-path blockers — **Blocker 1:** the live submit-mark now reads the **real broker GLD mark** (`live_submit_mark` → the read-only `data.alpaca.markets` `trade.p`/`trade.t`) inside the startup reconcile, fail-closed, never the sim derived proxy, so slippage is fill-vs-real-mark; **Blocker 2:** a new live-path-only **PendingOrder** kind on SCHEMA-015 (omitted-when-empty → no `schema_version` bump / no benchmark re-pin) lets the next `operate_live` startup reconcile **fold a cross-run async fill exactly once** (also fixing FLAT-misclassification-as-discrepancy). Both live-plug-only: **zero golden movement** (`git hash-object` byte-identical), full suite **1137 green**, `mypy --strict`/`ruff` clean on touched files, the `assert port.replayable` fence holds. B-flags doc-verified vs Alpaca docs (B1 REST max is **128** — the 48 cap is the FIX limit, self-imposed-safe; B3 `cash`+`multiplier`, the lifecycle facts, and the `trade.p`/`trade.t` mark fields **confirmed**); `scripts/alpaca_paper_e2e_probe.py` readied for the empirical-pending items (GLD `fractionable`, duplicate→422 body, cash settlement). No new dev_graph node (additive edits to SCHEMA-015 + FILE-044/045). **Arming ≠ trading:** the decision layer still emits only AVOID (ADR-012 DEFER) → AVOID → NO_ACTION → no order; enabling live stays the operator's ADR-013 gated-live action, `set_operator_halt` the governed stop. **Prior 2026-06-23 — ADR-014 slices 1-6:** the dormant Alpaca **paper** adapter is now operable — reconcile-then-act, FLAT→exit + realized P&L, deterministic idempotency, cash-cap, queued/partial/uncertain state machine, GLD-share `exec_ref` price fix, and an audited operator kill switch — all quarantined in the default-OFF non-replayable live plug behind the `assert port.replayable` fence and reached only via `live_runtime.operate_live`. +10 nodes: FILE-044 [[live_runtime.py]], FILE-045 [[live_adapter.py]], FILE-046 [[price_reference.py]]; TEST-034..040. SCHEMA-014/015 gained additive fields + the live-path-only **ReconcileEntry** kind (omitted-when-empty → sim/replay byte-identical). Two code version bumps (`PORTFOLIO_SCHEMA_VERSION` 0.2.0, `EXEC_PRICE_SOURCE_VERSION` 1.0.0) from slices 1-2 re-pinned BENCH-004/006; slices 3-6 added zero golden movement. Full suite **1128 green**; adversarial review 0 findings. Live execution + schedule registration stay operator HARD-PAUSE. **Prior 2026-06-18:** Two ADR-009 amendments + the Chain Orchestrator. **(A) Computed Cooldown** — PRED-008 [[Cooldown OK]]; MOD-007 `runtime_policy_version 0.1.0 → 0.2.0` (computed L3 cooldown replaces the v0 echo; duplicate-first); goldens BENCH-003/004/006 re-pinned (identity-only). **(B) Live Operational Feed** — FILE-036 [[operational_feed.py]] + TEST-026 under MOD-010 (deterministic MarketCalendarFeed behind the OperationalInput seam, captured for replay / quarantined off the replay path; additive — no contract/`*_version` change). Both **HARD-PAUSED for operator review, nothing committed**. **The Chain Orchestrator** — MOD-010 + FILE-031..035 + TEST-023..025 + BENCH-006 (committed golden chain_bench.json): the end-to-end Layer-3 composition root (consume→…→[GATE-001]→execute→persist, in-hand direction/gold_price ADR-011 D1); pure composition; realizes PAT-004, operationalizes WF-001. Full suite **950 green**. **Alpaca live plugs + chain scheduling + calibration re-check (2026-06-18):** FILE-037 [[alpaca_clock_feed.py]] + TEST-027 (AlpacaClockFeed, closes the v0 holiday-calendar gap, default-OFF) under MOD-010; FILE-038 [[alpaca_adapter.py]] + TEST-028 (AlpacaPaperAdapter, non-replayable / built-but-dormant, default-OFF) under MOD-008 — **ADR-011 gate (f) CLOSED**, ADR-009 live-feed amendment extended; both fail-closed, paper-only (refuse non-paper URL), creds env/`.secrets`-only (KA-008). Chain scheduling scripts built + manually validated on the **simulator** path (`scripts/daily_chain_run.ps1`); **registering the recurring task and enabling live execution are operator HARD-PAUSE actions — not done here**. ADR-012 readiness re-check: corpus still N=5 monochromatic RESTRICTIVE_RATES/AVOID → **all three targets DEFER, no bump** (golden unchanged). Adversarial review hardened the paper-only host check (substring → parsed-hostname equality, closing sub/super-domain/query/fragment/non-https bypasses). Full suite **986 green**.)
...
| [[Operations Control Plane Console]] | OBS-002 | observability | The local terminal operator console surface (MOD-011) — header/status bar + Overview/Gates/Calibration/Artefacts/Processes/Plugs/Log panes over the Layer-3 runtime; paper-only, secrets-never-shown (ADR-013) |
...
| [[Operations Control Plane]] | MOD-011 | module | Local terminal operator console (headless read-model + Textual TUI) over the Layer-3 runtime; three action tiers behind confirm/audit/server-side-precondition; reuse-governed-functions; paper-only/secrets-never-shown/zero-dep (ADR-013); `ops/` package, never imported by src/ |
...
| [[alpaca_clock_feed.py]] | FILE-037 | file | Live Alpaca clock/calendar OperationalFeed plug (MOD-010 seam) — AlpacaClockFeed (closes the v0 holiday gap via /v2/calendar), default-OFF, fail-closed, captured/quarantined; non-replayable sibling of MarketCalendarFeed |
| [[alpaca_adapter.py]] | FILE-038 | file | Live Alpaca paper ExecutionPort plug (MOD-008, gate f) — AlpacaPaperAdapter (mode=alpaca_paper, replayable=False), default-OFF / built-but-dormant, paper-only, fail-closed; non-replayable sibling of SimulatedBrokerAdapter |
| [[core.py (ops)]] | FILE-039 | file | Headless governed read-model (MOD-011) — assemble_dashboard() (no Textual); pure reads of ledger/portfolio/operational + pure preview + gate boards + calibration readiness (eligible flag) + plugs/processes/policy/audit-tail; fail-closed, secrets never shown |
| [[app.py (ops)]] | FILE-040 | file | Textual TUI (MOD-011) — OpsConsole + ConfirmModal; 7 panes; tiers as key bindings; crash-proof thread workers; `--once` headless dump; only file importing Textual/Rich |
| [[actions.py (ops)]] | FILE-041 | file | Tier-2 safe actions (MOD-011) — run-chain-now (simulator), re-run calibration readiness, re-sync Neo4j; reuse governed fns; audit-logged; never raise; full audit surface redacted |
| [[gated.py (ops)]] | FILE-042 | file | Tier-3 gated-live actions (MOD-011) — one-shot Alpaca-paper run (fail-closed REFUSE without paper creds/host), register/unregister schedule, commit-or-DEFER calibration bump (never bumps a `*_version`); server-side preconditions; never raise |
| [[audit.py (ops)]] | FILE-043 | file | Append-only ASCII audit log (MOD-011) — make_entry/write_entry/append_audit/read_audit over runtime/ops/audit_log.jsonl; secret-free; ADR-013 §7 |
...
| [[test_alpaca_clock_feed]] | TEST-027 | test | MOD-010 live Alpaca clock feed (FILE-037) — calendar open/closed, holiday-gap closure (Good Friday), fail-closed paths, from_env paper-only refusal incl. spoofed sub/super-domain/query/fragment/non-https URLs (parsed-hostname check), replay-path quarantine; mocked client, no network (17 tests) |
| [[test_alpaca_adapter]] | TEST-028 | test | MOD-008 live Alpaca paper adapter (FILE-038) — fill echo, mode/replayable flags, fail-closed (no client / non-LONG / unfilled), from_env paper-only refusal incl. spoofed URLs (parsed-hostname check), engine quarantine (replayable=False stamped, no-fill never calls broker); mocked broker, no network (20 tests) |
| [[test_ops_core]] | TEST-029 | test | MOD-011 read-model (FILE-039) — view shapes, pure-preview purity (no persistence), fail-closed loaders, calibration DEFER + eligible flag, plugs dormant w/o creds, no-credential-leak in to_dict/render (31 tests) |
| [[test_ops_audit]] | TEST-030 | test | MOD-011 audit log (FILE-043) — append/read round-trip, append-only, ASCII (ensure_ascii), tail limit, malformed-line skip (7 tests) |
| [[test_ops_actions]] | TEST-031 | test | MOD-011 Tier-2 actions (FILE-041) — run-chain persists+audits+idempotent, calib/neo4j audit on success+failure, never-raise (audit-write-fail), secret redaction (14 tests) |
| [[test_ops_gated]] | TEST-032 | test | MOD-011 Tier-3 actions (FILE-042) — Alpaca REFUSE when dormant, live port threaded w/o network (broker stub never called), bump DEFER-only/golden-unchanged, register/unregister, never-raise, no-secret (13 tests) |
| [[test_ops_app]] | TEST-033 | test | MOD-011 Textual TUI (FILE-040) — worker survives raising action (app not exited), gated confirm-modal flow (confirm runs+audits, cancel no-op), the ISSUE-07 adopt confirm flow + Live-tab render; headless run_test (5 tests) |
| [[test_live_monitoring]] | TEST-041 | test | MOD-011 LIVE read-model (FILE-039) + adopt action (FILE-042) — live ledger/portfolio read separate `*.live` files + independent of SIM, realized P&L (incl. carry-forward on adopt), reconcile discrepancy + derived refuse/adoptable, SIM/LIVE badging, no-secret, append-only precondition-gated adopt heals the refuse (24 tests) |
...
- **Total content nodes**: 212 (architecture: 4, system: 6, capability: 21 (incl. 1 deprecated), interface: 7, artifact_schema: 12, module: 11, file: 43, test: 34, gate: 3, predicate: 8, pattern: 11, workflow: 1, knowledge_asset: 11, governance: 7, reference: 3+1 deprecated, observability: 2, context_pack: 2, decision_record: 14, constraint: 3, api_doc_source: 2, benchmark_result: 6)
...
- **Frontmatter coverage**: 222/222 content nodes (100%)
- **Canonical ID coverage**: 222/222 content nodes (100%)
...
- **Alpaca live plugs (ADR-011 gate f) + chain scheduling + ADR-012 readiness re-check — FILE-037/038 + TEST-027/028 date**: 2026-06-18 — `AlpacaClockFeed` (FILE-037, `src/orchestration/alpaca_clock_feed.py`) behind the `OperationalFeed` port closes the v0 holiday-calendar gap via Alpaca `/v2/calendar` (paper base URL); `AlpacaPaperAdapter` (FILE-038, `src/execution/alpaca_adapter.py`) behind INT-011 `ExecutionPort` (`mode=alpaca_paper`, `replayable=False`). Both **default-OFF**, **fail-closed** (`*_from_env` refuse any non-paper URL + fail-closed on missing creds; creds env/git-ignored `.secrets` only per KA-008; PRED-005 enforced), **non-replayable / built-but-dormant** (the adapter cannot fill until a LONG exists, ADR-011 §5; never on `run_sequence`/benchmarks); CLI gained an additive `--operational-feed-source {calendar,alpaca}` (default `calendar`; `run_once`/`run_sequence` contracts unchanged); **ADR-011 §7 gate (f) CLOSED (all six now closed)**, ADR-009 live-feed amendment extended. **Chain scheduling** (Part 2): `scripts/daily_chain_run.ps1` (manually validated once on the deterministic **simulator** + calendar feed — latest banked snapshot `c1fe5a02` → RESTRICTIVE_RATES/AVOID → ADMIT + no-fill) + `scripts/register_daily_chain_task.ps1` (built, parse-checked, **NOT registered — operator HARD-PAUSE**); ops tooling, no file nodes (mirrors Mr-Ripley). **ADR-012 readiness re-check** (Part 3): corpus unchanged from 2026-06-17 (N=5, monochromatic RESTRICTIVE_RATES/AVOID, 0 realized labels) ⇒ **G0+G1/G2/G3 all FAIL → all three targets DEFER, NO `*_version` bump / value / direction-table change** (committed golden byte-identical). 36 new tests (TEST-027 17 + TEST-028 20, all mocked — no real network); full suite **986 green**; `mypy --strict` + `ruff` clean on the new files. **Adversarial review** (5-dimension workflow) found the paper-only credential boundary used a weak **substring** URL check; **hardened** to a parsed-hostname equality check (`urlparse().hostname == paper-api.alpaca.markets` + https), closing sub-/super-domain, query/fragment, and cleartext bypasses, with regression tests — fail-closed / quarantine / determinism / governance dimensions reviewed clean. **Enabling live execution + registering the recurring schedule remain operator HARD-PAUSE actions.**
- **ADR-014 Operable Alpaca Paper Execution Adapter v1 — slices 1-6 (FILE-044..046 + TEST-034..040; SCHEMA-014/015 additive; MOD-008/010 + FILE-036 updated) date**: 2026-06-23 — the dormant Alpaca **paper** adapter turned operable as 6 additive vertical slices, all broker realism quarantined in the default-OFF non-replayable plug behind `live_runtime.operate_live` + the `assert port.replayable` fence. (i) **Shared/versioned (slices 1-3):** the source-pluggable GLD-share `exec_ref_gld_price` (derived proxy `gold_price_proxy × OZ_PER_SHARE` on sim/replay, live mark on Alpaca; `gold_price` severed from execution; new `EXEC_PRICE_SOURCE_VERSION 1.0.0`); the daily-guard `as_of` threading + `ExecutionEntry.as_of` (`PORTFOLIO_SCHEMA_VERSION 0.1.0→0.2.0`, day-scopes `trades_today`); the `assert port.replayable` fence on `run_once`/`run_sequence` + separate live `(ledger, portfolio)` paths — **BENCH-004/006 re-pinned for the two versioned keys only**. (ii) **Live-plug-only (slices 4-6):** the side/order-based `LiveExecutionAdapter` + QUEUED/PARTIAL/EXECUTION_UNCERTAIN state machine + typed errors + 429 + account-status gate + cash-cap + `client_order_id` 422 dedup (FILE-045); `operate_live` reconcile-then-act — mandatory startup reconcile, side-based buy/sell/NO_ACTION on the netted delta, discrepancy → terminal-refuse + append-only **ReconcileEntry** heal (no auto-flatten), live SELL-fold realized P&L (sim stays accumulate-only), idempotency bifurcation (FILE-044); the audited Tier-2 operator kill switch (`OperatorHaltFeed` + `set_operator_halt`, honored via the existing `operational_ok`). **Determinism binding & proven:** `to_dict` omits an empty `reconciles` so sim/replay portfolios stay byte-identical (slices 3-6 = zero golden movement, `git hash-object` unchanged); no live read enters the shared guard / `run_chain` / simulator. Full suite **1091→1128 green** (+37 tests, all stub-driven — no network); adversarial 4-dimension review **0 confirmed findings**. **Enabling live execution + registering the recurring schedule remain operator HARD-PAUSE; calibration bumps stay DEFER until the ADR-012 gate passes; empirical flags B1/B2/B3 confirm against Alpaca docs before any live run.**
